...
const getSubasta = async (request, response) => {
	await pool.query(
...
const putSubasta = async (request, response) => {
	await pool.query(
		'SELECT  * FROM "AA_Subasta_Evento" where id = ' + request.subastaId,
		(error, results) => {
			if (error) {
...
const getCatalogosMoneda = async (request, response) => {
	await pool.query(
		'SELECT * FROM  "AA_Catalogo_Moneda" inner join "AA_Moneda" on "AA_Catalogo_Moneda".moneda="AA_Moneda".id ',
		(error, results) => {
			if (error) {
				throw error;
			}
...
const createSubasta = async (req, res) => {
	...
	const response = await pool.query(
		...
		[fecha, duracion, costo_inscripcion, costo_cliente, pais, tipo]
	);
	...
	for (const coleccionista of coleccionistas) {
		const response2 = await pool.query(
			'INSERT INTO "AA_Participante"(subasta,coleccionista) values ((SELECT  id as subasta FROM "AA_Subasta_Evento"ORDER BY id DESC limit 1),$1)',
			[coleccionista]
		);
	}
...
const createCostoEnvio = async (request, response) => {
	...
	await pool.query(
		'INSERT INTO "AA_Costo_Envio"'[
			(date, duracion, costo_inscripcion, costo_inscripcion_cliente, tipo)
		],
		(error, results) => {
			if (error) {
				throw error;
