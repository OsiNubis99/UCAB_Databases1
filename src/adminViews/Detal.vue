	...
	>
		<template v-slot:top>
			<v-toolbar flat color="white">
				<v-toolbar-title>Tiendas al Detal</v-toolbar-title>
							...
							<v-container grid-list-md>
								<v-layout wrap v-if="editedIndex != -2" justify-center>
									...
									<v-layout>
										...
										<v-btn
											color="primary"
											@click="cargarFoto('Detales/' + editedItem.id)"
										>
...
export default {
	...
	methods: {
		...
		editItem(item) {
			this.editedIndex = this.Detal.indexOf(item);
			this.editedItem = Object.assign({}, item);
			this.dialog = true;
		},
		...
		deleteItem(item) {
			...
			confirm("Esta seguro que desea eliminar esta Tienda?") &&
				this.axios
					.get("https://www.ofimania.com/API/Tienda/Detal/Delete/" + item.id)
					.then((response) => {
						this.cargando = false;
						this.iniciar();
						this.$store.state.alerta = {
		...
		async save() {
			...
			if (this.editedIndex > -1) {
				await this.axios
					.post(
						"https://www.ofimania.com/API/Tienda/Detal/Update/" +
							this.editedItem.id,
				...
				await this.axios
					.post(
						"https://www.ofimania.com/API/Tienda/Detal/Add",
						qs.stringify(this.editedItem),
						{
							headers: {
								"Content-Type": "application/x-www-form-urlencoded",
