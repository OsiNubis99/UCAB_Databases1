						...
						<v-card-text>
							<v-container grid-list-md>
								<v-layout wrap v-if="editedIndex != -2" justify-center>
									...
									<v-flex xs12 lg6 v-if="editedIndex != -1">
										<v-text-field
									...
									<v-layout>
										...
										<v-btn
											color="primary"
											@click="cargarFoto('Blog/' + editedItem.id)"
										>
	...
	methods: {
		...
		deleteItem(item) {
			...
			confirm("Esta seguro que desea eliminar esta Tienda?") &&
				this.axios
					.get("https://www.ofimania.com/API/Blog/Delete/" + item.id)
					.then((response) => {
						this.cargando = false;
						this.iniciar();
						this.$store.state.alerta = {
		...
		ordenar(position, index) {
			this.cargando = true;
			this.axios
				.get("https://www.ofimania.com/API/Blog/" + position + "/" + index)
				.then((response) => {
					this.cargando = false;
					this.$store.state.alerta = {
						estado: true,
		...
		iniciar() {
			this.cargando = true;
			this.axios
				.get("https://www.ofimania.com/API/Blog/GetAll")
				.then((response) => {
		...
		async save() {
			...
			if (this.editedIndex > -1) {
				await this.axios
					.post(
						"https://www.ofimania.com/API/Blog/Update/" + this.editedItem.id,
				...
				await this.axios
					.post(
						"https://www.ofimania.com/API/Blog/Add",
						qs.stringify(this.editedItem),
						{
							headers: {
								"Content-Type": "application/x-www-form-urlencoded",
