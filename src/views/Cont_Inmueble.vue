  ...
  >
	  <template v-slot:top>
			<v-toolbar flat color="white">
				<v-toolbar-title>Contratos de Inmuebles</v-toolbar-title>
						...
						<v-card-text>
							<v-container grid-list-md>
								<v-layout wrap v-if="editedIndex != -2" justify-center>
									<v-flex xs12 lg6>
										<v-select
											v-model="editedItem.id_inmueble"
											:items="inmuebles_names"
											label="Secciona el Inmueble"
										>
...
export default {
	...
	computed: {
		inmuebles_names() {
			return this.Inmuebles.map(function(item) {
				return [item.id_inmueble, item.direc_inmueble];
			});
		},
		clientes_names() {
	...
	methods: {
		...
		iniciar() {
			this.cargando = true;
			this.axios
				.get("http://localhost:4000/contratos_Inmueble")
				.then((response) => {
					this.cargando = false;
					this.Contratos = response.data.Contratos;
					this.Inmuebles = response.data.Inmuebles;
		...
		async save() {
			console.log('test')
			var qs = require("qs");
			this.editedItem.id_inmueble = this.editedItem.id_inmueble[0];
			...
			if (this.editedIndex > -1) {
				...
				await this.axios
					.post(
						"http://localhost:4000/contrata_Inmueble",
						qs.stringify(this.editedItem),
						{
							headers: {
								"Content-Type": "application/x-www-form-urlencoded",
